...
##  1.
###
def visit_type_category(struct_type, new_prop, type_dict, structure_svc, enum_svc, ref_path):
...
def visit_generic(generic_instantiation, new_prop, type_dict, structure_svc, enum_svc, ref_path):
    ...
    if generic_instantiation['generic_type'] == 'OPTIONAL':
        ...
        if generic_instantiation['map_key_type']['category'] == 'USER_DEFINED':
            ...
            check_type(res_type, res_id, type_dict, structure_svc, enum_svc, ref_path)
        else:
        ...
        if generic_instantiation['map_value_type']['category'] == 'USER_DEFINED':
            ...
            visit_generic(generic_instantiation['map_value_type']['generic_instantiation'],
                          new_type['properties']['value'], type_dict, structure_svc, enum_svc, ref_path)
        new_prop['type'] = 'array'
        new_prop['items'] = new_type
        if '$ref' in new_prop:
            del new_prop['$ref']

def visit_user_defined(user_defined_type, newprop, type_dict, structure_svc, enum_svc, ref_path):
    ...
    check_type(user_defined_type['resource_type'], user_defined_type['resource_id'], type_dict, structure_svc, enum_svc, ref_path)
...
##  2.
###
def check_type(resource_type, type_name, type_dict, structure_svc, enum_svc, ref_path):
    if type_name in type_dict or is_type_builtin(type_name):
        return
    if resource_type == 'com.vmware.vapi.structure':
        structure_info = structure_svc[type_name]
        if structure_info is not None:
            ...
            type_dict[type_name] = {}
            process_structure_info(type_name, structure_info, type_dict, structure_svc, enum_svc, ref_path)
        else:
            eprint("Could not fetch structure info for " + struct_type)
    else:
        enum_info = enum_svc[type_name]
...
def process_structure_info(type_name, structure_info, type_dict, structure_svc, enum_svc, ref_path):
    new_type = {'type': 'object', 'properties': {}}
